...
class CellStainIntensityGaussian(base.SimulationBase):
    ...
    def run_task(self, config, simParams):
        ...
        for zi in range(simParams.get_number_z()):
            ...
            # locImage is expected to 1.0 or greater where there
            # is cell stain present.
            #
            mask = (locImage >= 1.0)

            # Scale up by mean intensity.
            locImage = locImage * self._parameters["intensity_mean"]

            # Add Gaussian noise.
            noise = np.random.normal(0.0,
                                     self._parameters["intensity_sigma"],
                                     locImages[zi].shape)
            locImage[mask] += noise[mask]

            # Clip out values less than 1.0 as we may use this image
            # as the lambda of a Poisson distribution.
            locImage = np.clip(locImage, 1.0, None)
...
class CellStainGradient(base.SimulationBase):
...
class DAPIGradient(CellStainGradient):
    """
    DAPI dyes with spatial gradient.
    """
...
class PolyTGradient(CellStainGradient):
    """
    PolyT dyes with spatial gradient
    """
    def __init__(self, **kwds):
        super().__init__(**kwds)
